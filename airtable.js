...
// This function runs on Netlify's servers, not in the browser. That solves two
// problems with talking to Airtable directly from client-side JavaScript:
//
//   1. CORS — Airtable's API does not allow direct browser-to-Airtable requests
//      from arbitrary websites. Server-to-server calls (like this one) aren't
//      subject to that restriction.
//   2. Secrecy — your Airtable token lives only here, as a Netlify environment
//      variable. It is never sent to, or visible in, the public HTML page.
//
// The browser sends a small JSON request to this function; this function talks
// to Airtable on its behalf and sends back the result.
//
// Required Netlify environment variables (set in Site configuration → Environment
// variables — NOT in this file, and NOT in netlify.toml):
//   AIRTABLE_TOKEN   — personal access token from airtable.com/create/tokens
//   AIRTABLE_BASE    — the base ID, looks like appXXXXXXXXXXXXXX
...
exports.handler = async (event) => {
  ...
  if (!AIRTABLE_TOKEN || !AIRTABLE_BASE) {
    // Not configured yet — this is a normal, expected state before setup is
    // finished. The app falls back to browser-only storage when it sees this.
